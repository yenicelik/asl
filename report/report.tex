...
I run 3 repetitions of each configuration, each having a length of 90 seconds (such that a warm-up and cool-down time of 15 seconds respectively evens out).
...
%TODO Merge the next two subsections
...
The throughput reached approximately almost $18000ops/s$, which conforms to the observation of the maximum throughput per server in section 1.
...
For write-only operations, the bottleneck is the upload bandwidth of the thee clients,
...
I first present the graph. 
The throughput reached approximately almost $6000ops/s$, which conforms to the observation of the maximum throughput per server machine in section 1.
...
For read-only operations, the bottleneck is the upload bandwidth of the server virtual machines,
...
\subsubsection{Write-only}
Again, I first present the graph.
The throughput reached approximately almost $6000ops/s$, which conforms to the observation of the maximum throughput per client machine in section 1.
This means that both client virtual machine, are using their maximum capacity to upload all the values to the server virtual machines.
The combined throughput of the two servers is $2 \times 100Mbit/s$ which corresponds to a compound $200Mbit/s$.
This conform to the $6000ops/sec$ when using the same calculation as in section 1.2.
Please notice that the error is again very small, and thus intersects with the measured points.
The error metric is explained in section 1.
...
For write-only operations, the bottleneck is the upload bandwidth of the client virtual machines, as the single client is uploading 200Mbit/s each, which compounds to 200Mbit/s together.
The sane expectation would be a throughput of 6'000 ops/sec (as given per the bandwidth of the client virtual machine).
However, notice that the memtier client json files return values of 6'000 ops/sec each. 
However, the throughput of this operation is at 12'000 ops/sec as measure by the client, which does not immediately make sense, and is an issue that is very difficult to debug.

After careful analysis, I arrive at the conclusion that this may be due to one of the following two factors. 
1. When I replicate the memtier-instances, the confirmation is sent from the server to both memtier-instances (which would mean that the throughput is doubled).
I run one additional test to check what would happen when we have a single server.
The total throughput of the system halves (as we have halved the servers).
This indicates that the replicated confirmaitons may indeed be an issue causing the higher-than-expected throughput.
This is unfortunately one factor that I cannot solve without changing the program of memtier, or modifying the virtual-machine setups.
2. Microsoft Azure may be responsible for some internal handling of the virtual-machine resource allocations, allowing for a higher bandwidth.
Unfortunately, I cannot analyse this effectively as I do not have access to Microsoft Azure's policy of resource allocation, so explanation 1. is the only good explanation for this unexpected behavior. \\

The server can respond fast enough to all requests by the clients, as long as there is only up to 8 virtual clients per thread, because the response message (which is usually "STORED") consumes almost none of the bandwidth.
Once the client spawns 8 virtual clients per thread, the server VM bandwidth starts to saturate as the client upload bandwidth of 200Mbit/s is reached.
The system becomes saturated after an increase of virtual client threads, as can be seen from the plot which indicates that there is no oversaturation.
Because the network bandwidth stays constant, the round-trip time of individual requests increases, except for up to 4 virtual clients per thread.
For 4 virtual clients per threads, the response time stays constant, simply due to the fact that the bandwidth is not saturated, and the requests don't have to wait on each other to be processed (but can be processed in parallel).
This implies an almost constant latency for up to 4 virtual clients per thread, and then an increase in latency, as can be seen from the graph.
The latency and throughput graphs show a correct, inverse correlation.
