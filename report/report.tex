...
\caption{Exp5.1: Latency in ms as a function of multi-get \textbf{keysize} and \textbf{percentile amongst all requests}. The labels on the x-axis are to be interpreted by "\textbf{keysize} : \textbf{percentile}" (i.e. the element before the semicolon denotes the keysize, the element after the semicolon denotes the percentile). This graph only shows \textbf{sharded} requests.  }
\label{fig:test}
\end{figure}
...
Any configuration that I do not specify further is exactly the same as in the sharded case.
...
\caption{Exp5.1: Latency in ms as a function of multi-get \textbf{keysize} and \textbf{percentile amongst all requests}. The labels on the x-axis are to be interpreted by "\textbf{keysize} : \textbf{percentile}" (i.e. the element before the semicolon denotes the keysize, the element after the semicolon denotes the percentile). This graph only shows \textbf{non-sharded} requests.}
\label{fig:test}
\end{figure}
...
As my byteBuffer is of size $15 * 4024$, and a single "full" datapacket for message is of size $ 4096 $, this easily fits into the byteBuffer (again, for the same reason as the sharded case).
...
\begin{subfigure}{.5\textwidth}
    ...
    \label{fig:mesh1}
\end{subfigure}
\caption{Exp5.1: Latency and throughputs as measures by the clients}
\label{fig:test}
\end{figure}
...
%TODO average queue length and average wait time for each request?
...
After considering taking the mean of multiple repetitions, I arrived at the conclusion of only picking a single memtier-instance's output and plotting this. 
...
\begin{subfigure}{.5\textwidth}
    \centering
    \includegraphics[width=\textwidth]{img/exp5_1/exp_5_1_histogram_mw_nonsharded.png}
    \caption{Non-Sharded Latency Histogram}
    \label{fig:mesh1}
\end{subfigure}%
\begin{subfigure}{.5\textwidth}
      \centering
    \includegraphics[width=\textwidth]{img/exp5_1/exp_5_1_histogram_mw_sharded.png}
    \caption{Sharded Latency Histogram}
    \label{fig:mesh1}
\end{subfigure}
\caption{Exp.5.1: Latency and throughputs as measures by the \textbf{middlewares}}
\label{fig:test}
\end{figure}
...
\begin{subfigure}{.5\textwidth}
    \centering
    \includegraphics[width=\textwidth]{img/exp5_1/exp5_1_histogram_client_nonsharded.png}
    \caption{Non-Sharded Latency Histogram}
    \label{fig:mesh1}
\end{subfigure}%
\begin{subfigure}{.5\textwidth}
      \centering
    \includegraphics[width=\textwidth]{img/exp5_1/exp5_1_histogram_client_sharded.png}
    \caption{Sharded Latency Histogram}
    \label{fig:mesh1}
\end{subfigure}
\caption{Exp.5.1: Latency and throughputs as measures by the \textbf{clients}}
\label{fig:test}
\end{figure}

As a sanity check, the minimum response time of the client is higher than the minimum response time of the middlewares.
This is simply because the client measures an additional network round-trip time, whereas the middleware only includes the time that it takes from the incoming request (after the request has gone through the network once), through the memcached server's waiting time, back to the middleware.
On this very last step again, the network trip time back to the client is neglected.
These times from client to middleware, and from middleware back to clients (stripping out the processing and waiting time) is not measured by the the middleware, however is measured by the client.
These measures add up to the latency, as can be seen on histograms.
The histograms support this claim, as there are some few requests that only take up to 1 millisecond, whereas on the client side, there are no requests taking up only 1 milliscond.
...
For the M/M/m case, I choose the mean service rate to be the \textbf{throughput per middleware thread}, which I calculate by taking dividing the total middleware throughput by the number of threads.
The logic behind this is that each individual process is suppsoed to handle each request separately, up to the part which ends up in the queue.
Of course, the load that ends up in the queue makes up a big fraction of the total lifestime of a request, but this is the closest approximately that I can find about multiple services in the system.
\item For the M/M/m model $m$: I use the number of middleware threads as the number of services, motivated for the same reasons as above.
I do this because each middlewarethread is supposed to do parallel work (up to the queue size).
\end{enumerate}
...
For the M/M/m model, the service rate will be divided by the total number of workerthreads as these threads separately work on the total workload.
...
The comparisong comparison is conducted by calculating the relative error between observed and predicted values. 
The error is calculated using the following formula:

\begin{equation}
e = \frac{x - y}{y} 
\end{equation}
where $x$ is the real value, and $y$ the predicted value, and $e$ denotes the relative error.
For the relative error, the close the error is to the value $0.$, the more accurate our predictions are.
Please notice that this error term is asymmetric.
...
\begin{table}[H]
	\caption{M/M/1 Predicted Values}
		\center
	\scriptsize{
		\begin{tabular}{|l|c|c|c|c|c|c|c|c|c|c|}
					\hline & & &  & Predicted & Predicted & Predicted \\ 
			\hline Middelware threads & $\mu$ & $\lambda$ & $\rho$ & Queue length & Latency in ms & Wait time in ms \\ 
			\hline 8 & 8278 & 8120 & 0.9809023 & 
			50.38 & 6.32536 & 6.20456 \\ 
			\hline 16 & 10491 & 9209 & 0.87780868 & 
			6.31 & 0.78007 & 0.68475\\ 
			\hline 32 & 11810 & 11789 & 0.99885762 & 
			1796.46 & 152.72627 & 152.64135 \\ 
			\hline 64 & 12984 & 12150 & 0.93577961 
...
The following table compares the observed values and the predicted values.
This comparison is conducted by calculating the relative error, which I predict using the following formula:
...
%TODO calculate all errors again

\begin{table}[H]
	\caption{M/M/1 Observed values and prediction error for each value. The error is always a \textbf{relative error}.}
		\center
	\scriptsize{
		\begin{tabular}{|l|c|c|c|c|c|c|c|c|c|c|}
					\hline & Observed & Observed & Observed & Error & Error & Error \\ 
			\hline & Queue length & Latency in ms & Wait time in ms & Queue length & Latency & Wait time \\ 
			\hline 8 & 
			33.66 &  9.737 & 1.72 &
			0.49673203 & -0.35037897 & 2.60730233 \\ 
			\hline 16 & 
			2.40 &  3.95 & 2.90 &
			1.62916667 & -0.80251392 & -0.76387931  \\ 
			\hline 32 &
			48.51 &  13.61 & 4.97 &
			36.03277675 & 10.2216216 & 29.71254527 \\ 
			\hline 64 &
			13.64 & 11.19 &  8.76 &
			-0.00073314 & -0.89282663 & -0.87188813 \\ 
			\hline
		\end{tabular}
	} 
\end{table}

One can see that the relative error is very high for almost all, expect 2 values.
As such, the M/M/1 queueing model provides a bad approximately to the throughput of the system.

I will now proceed with the approximations arrived by using the M/M/m queuing model.
...
The relative error gives a good measure of how different the observed values are from the measured values.
Although there are some values which are off by less than a few percent (i.e. $-0.0007$ implies being off by $ 0.7 \% $), most values are above $30\%$ off, or even off by a multiple of 30.
The queueing model for M/M/1 gives a good understand of the range in which order of magnitude the quantities will be, but for this system, is not a very good approximation of the exact numbers.
Some differences include that the model has high over-predictions for 32 middlewarethreads for example, where the queue-length is at 1796.
Most of the estimates for the queuelengths are quite accurate (up to $50 - 33 = 17$ items in the queue). 
The prediction for response times are not very accurate, but do provide an intuition on the order of magnitude.

\subsection{M/M/m}

\begin{table}[H]
	\caption{M/M/m Predicted Values}
		\center
	\scriptsize{
		\begin{tabular}{|l|c|c|c|c|c|c|c|c|c|c|}
					\hline & & & & Predicted & Predicted & Predicted \\ 
			\hline Middelware threads $m$ & $\mu$ & $\lambda$ & $\rho$ & Queue length & Latency in ms & Wait time in ms \\ 
			\hline 8 & 509 & 8120 & 0.995 & 
			211.4 & 28.00 & 26.04 \\ 
			\hline 16 & 309 & 9209 & 0.930 & 
			7.98 & 4.100 & 0.866 \\ 
			\hline 32 & 183 & 11789 & 	0.999 & 
			1787 & 157 & 151 \\ 
			\hline 64 & 101 & 12150 & 0.9357 & 
			5.14 & 10.28 & 0.42  \\ 
			\hline
		\end{tabular}
	} 
...
\begin{table}[H]
	...
	\scriptsize{
		\begin{tabular}{|l|c|c|c|c|c|c|c|c|c|c|}
			...
			\hline 8 & 
			33.66 &  9.737 & 1.72 &
			5.28045157 & 1.87562904 & 14.13953488 \\ 
			\hline 16 & 
			2.40 &  3.95 & 2.90 &
			2.325    &   0.03797468  & -0.70137931  \\ 
			\hline 32 &
			48.51 &  13.61 & 4.97 &
			35.83776541 & 10.53563556 & 29.38229376 \\ 
			\hline 64 &
			13.64 & 11.19 &  8.76 &
			-0.62316716 & -0.08132261 & -0.95205479 \\ 
			\hline
		\end{tabular}
	} 
\end{table}
