...
def create_queue_barplots():
    ...
    #
    time_labels = ['Time to Enqueue', 'Time in Queue', 'Time Queue to Server', 'Time at Server',
                   'Time Server to Client']
    ...
    for write in [0, 1]:
        ...
        for _vc in range(0, 6):
            ...
            middleware_latencies = np.zeros(
                (total_middleware_threads, len(time_labels), 1, 3))  # 3 repetitions, 2 middlewares
            ...
            create_multiple_histogram_plot(
                ...
                means=mw_means.T,
                stddevs=mw_stddev.T,
                filepath=GRAPHPATH + "exp3_1_mw_percentile_plots_writes_{}__vc_{}".format(write, vc),
                is_queue=True
            )
